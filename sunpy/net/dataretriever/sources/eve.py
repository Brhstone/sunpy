...
__all__ = ['EVEClient', 'EVELeve2BClient']
...
BASEURLHIGHLEVEL = ('http://lasp.colorado.edu/eve/data_access/evewebdataproducts/'
           'level2b/%Y/%j/EV{product}_{level_short}_%Y%j_{version:03d}_{revision:02d}.fit.gz')

class EVELeve2BClient(GenericClient):
    """
    This EVEClient is for the Level 0C data
    from http://lasp.colorado.edu/home/eve/data/data-access/.
    ...
    def _get_url_for_timerange(self, timerange, **kwargs):
        ...
        if timerange.start.time() != datetime.time(0, 0):
            timerange = TimeRange('{:%Y-%m-%d}'.format(timerange.start), timerange.end)
        eve = Scraper(BASEURLHIGHLEVEL, level_short='L2B', product='S', version=6, revision=2)  # TODO: Dont do twice
        return eve.filelist(timerange)

    def _get_time_for_url(self, urls):
        eve = Scraper(BASEURLHIGHLEVEL, level_short='L2B', product='S', version=6, revision=2)
        times = list()
        for url in urls:
            t0 = eve._extractDateURL(url)
            # hard coded full day as that's the normal.
    ...
    def _can_handle_query(cls, *query):
        ...
        for x in query:
            ...
            elif x.__class__.__name__ == 'Level' and isinstance(x.value, str) and x.value.upper() == 'L2B':
                chk_var += 1
